...
export default async function FaturaPage({ params }: { params: Promise<{ locale: string, id: string }> }) {
  ...
  const booking: any = await prisma.bookings.findUnique({
    ...
    include: {
      ...
      businesses: true,
      payments: true // <--- SHTUAR: Pa këtë fatura nuk i sheh pagesat/rimbursimet
    }
  });
  ...
  return <FaturaClient booking={safeData} locale={locale} />;
}