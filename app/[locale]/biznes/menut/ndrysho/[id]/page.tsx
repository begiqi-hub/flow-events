...
import { Utensils, Banknote, Image as ImageIcon, ArrowLeft, Save, AlignLeft, Wallet, Info } from "lucide-react";
...
export default function EditMenuPage({ params }: { params: Promise<{ locale: string, id: string }> }) {
  ...
  const t = useTranslations("EditMenu");
  ...
  const [formData, setFormData] = useState({
    name: "", price_per_person: "", internal_cost: "", description: "", image: ""
  });
      ...
      <form onSubmit={handleSubmit} className={`bg-white rounded-3xl border border-gray-100 shadow-sm p-6 md:p-10 flex flex-col transition-opacity ${fetching ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
        ...
        <div className="mb-8">
          <label className="flex items-center gap-2 text-sm font-medium text-gray-500 mb-2">
            <Utensils size={16} /> {t("menuNameLabel")}
          </label>
          <input type="text" required placeholder={t("menuNamePlaceholder")} className="w-full border border-gray-200 p-4 rounded-xl outline-none focus:border-gray-900 focus:ring-1 font-medium text-gray-900" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-4 rounded-xl border border-gray-100 mb-8">
          <div>
            ...
            <input type="number" step="0.01" placeholder="Psh: 12.50" className="w-full border border-gray-200 p-3 rounded-xl outline-none focus:border-orange-500 focus:ring-1 bg-white" value={formData.internal_cost} onChange={(e) => setFormData({...formData, internal_cost: e.target.value})} />
          </div>

          <div>
            <label className="flex items-center gap-2 text-sm font-bold text-gray-900 mb-2">
              <Banknote size={16} className="text-emerald-500" /> {t("priceLabel")} (Çmimi Shitjes)
            </label>
            <input type="number" step="0.01" required placeholder={t("pricePlaceholder")} className="w-full border border-emerald-200 p-3 rounded-xl outline-none focus:border-emerald-500 focus:ring-2 bg-white" value={formData.price_per_person} onChange={(e) => setFormData({...formData, price_per_person: e.target.value})} />
          </div>
        </div>
